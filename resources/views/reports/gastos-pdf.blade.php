...
.stat-box { width: 25%; border-radius: 3px; padding: 5px 8px; text-align: center; vertical-align: top; }
...
thead th { padding: 3px 4px; text-align: left; font-size: 7px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.3px; border: 1px solid #164172; white-space: nowrap; }
...
.badge { padding: 1px 5px; border-radius: 99px; font-size: 6.5px; font-weight: 700; }
...
.footer { margin-top: 8px; border-top: 1px solid #d1d9e6; padding-top: 3px; font-size: 6.5px; color: #9ca3af; }
...
<table style="width:100%;border-collapse:collapse;margin-bottom:6px;">
    <tr>
        ...
        <td style="background:#1E3A5F;padding:8px 12px;text-align:right;vertical-align:middle;">
            <div style="font-size:12px;font-weight:bold;text-transform:uppercase;letter-spacing:0.5px;color:#fff;">Reporte de Gastos</div>
...
<table class="stats-wrap">
    <tr>
        ...
        <td class="stat-box stat-box--green">
            ...
            <div class="stat-value num">S/ {{ number_format($resumen['aprobados'], 2) }}</div>
            <div class="stat-sub">gastos confirmados</div>
        </td>
        <td class="stat-box stat-box--amber">
            <div class="stat-label">Pendientes</div>
            <div class="stat-value num">S/ {{ number_format($resumen['pendientes'], 2) }}</div>
...
<table class="main">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td class="num">{{ $g->fecha?->format('d/m/Y') ?? '—' }}</td>
            <td><span class="badge badge-cat">{{ $catLabel }}</span></td>
            <td>{{ Str::limit($g->descripcion, 55) }}</td>
            <td>{{ $g->empleado?->name ?? '—' }}</td>
            <td class="right num" style="font-weight:700">S/ {{ number_format((float)$g->monto, 2) }}</td>
        ...
        <tr class="totals-row">
            ...
            <td class="right num">S/ {{ number_format($resumen['suma'], 2) }}</td>
            <td colspan="2"></td>
        </tr>
    </tbody>
</table>
...
<div class="footer">
    <span>{{ $empresa->nombre }} — RUC {{ $empresa->ruc ?? '' }} &nbsp;|&nbsp; Reporte generado por SAS-PDV · {{ now()->format('d/m/Y H:i') }} · {{ $usuarioNombre }}</span>
</div>
