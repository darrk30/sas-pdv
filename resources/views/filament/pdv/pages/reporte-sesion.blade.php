...
<div class="vs-root">
    ...
    {{-- ══ CABECERA DE PÁGINA ══ --}}
    <div class="cc-modal__head" style="border-radius:.75rem;margin-bottom:1rem">
        ...
        <a href="{{ \App\Filament\Pdv\Pages\CierresCajaPage::getUrl() }}" wire:navigate
           class="cc-modal__close" title="Volver a Cierres de Caja" style="text-decoration:none">
    ...
    {{-- Skeleton mientras carga el tab --}}
    <div wire:loading wire:target="setTab,setSubTabVentas,setSubTabMov" class="rs-skel">
        <div class="rs-skel-kpis">
            @for($i = 0; $i < 8; $i++)<div class="rs-skel-kpi"></div>@endfor
        </div>
        <div class="rs-skel-rows">
            @foreach([100, 78, 92, 65, 85, 55] as $w)
            <div class="rs-skel-row" style="width:{{ $w }}%"></div>
            @endforeach
        </div>
    </div>

    {{-- Contenido real (oculto durante la carga) --}}
    <div wire:loading.remove wire:target="setTab,setSubTabVentas,setSubTabMov">
                    ...
                    <tbody>
                        @foreach($ventasTab as $v)
                            ...
                            <tr wire:key="vtab-{{ $v->id }}">
                                ...
                                <td class="rg-td-right" style="font-size:.78rem;color:var(--vs-text-muted)">{{ $v->detalles_count }}</td>
                                <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($v->total, 2) }}</span></td>
                                <td class="rg-td-right"><span class="rg-monto rg-monto--costo">S/ {{ number_format($v->costo_total, 2) }}</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                            ...
                            <tr wire:key="prod-{{ $loop->index }}">
                                ...
                                <td class="rg-td-right">
                                    <span class="cc-qty-badge">
                                        {{ $prod->qty == floor($prod->qty) ? number_format($prod->qty, 0) : number_format($prod->qty, 3) }}
                                    </span>
                                </td>
                                <td class="rg-td-right" style="color:var(--vs-text-muted);font-size:.78rem">
                                    {{ $prod->en_ventas }} venta{{ $prod->en_ventas != 1 ? 's' : '' }}
                                </td>
                                <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($prod->tot, 2) }}</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                        ...
                        <tbody>
                            @foreach($metTab['metodos'] as $met)
                                ...
                                <tr wire:key="met-{{ $loop->index }}">
                                    ...
                                    <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($met['sistema'], 2) }}</span></td>
                                    <td class="rg-td-right">
                                        @if($cajero !== null)
                                    ...
                                    <td class="rg-td-right" style="color:var(--vs-text-muted);font-size:.78rem">{{ $met['count'] }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                            ...
                            @foreach($metTab['comprobantes'] as $comp)
                                @php
                                    $tipoLabel = match(strtolower($comp->tipo)) {
                                        ...
                                        'ticket'  => 'Ticket / Nota', default => ucfirst($comp->tipo),
                                    };
                                    $tipoClass = match(strtolower($comp->tipo)) {
                                        'factura' => 'cc-badge--blue', 'boleta' => 'cc-badge--green',
                                ...
                                <tr wire:key="comp-{{ $loop->index }}">
                                    ...
                                    <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($comp->total, 2) }}</span></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
                            ...
                            <tr wire:key="cp-{{ $loop->index }}">
                                ...
                                <td class="rg-td-right">
                                    <span class="cc-qty-badge cc-qty-badge--gift">
                                        {{ $cp->qty == floor($cp->qty) ? number_format($cp->qty, 0) : number_format($cp->qty, 3) }}
                                    </span>
                                </td>
                                <td class="rg-td-right" style="color:var(--vs-text-muted);font-size:.78rem">
                    ...
                    <tbody>
                        @foreach($cortVentas as $cv)
                            <tr wire:key="cv-{{ $cv->id }}">
                                ...
                                <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($cv->total, 2) }}</span></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
    ...
    <div class="cc-tab-body">
        ...
        @php
            $movLabel = match($subTabMov) {
                'ing_apr' => 'ingresos aprobados', 'ing_anu' => 'ingresos anulados',
                'egr_apr' => 'egresos aprobados',  'egr_anu' => 'egresos anulados',
                default   => 'movimientos',
            };
        @endphp
    ...
    <div class="cc-tab-body">
        ...
        @php
            $dur = $sesInfo->fecha_cierre
                ? $sesInfo->fecha_apertura->diff($sesInfo->fecha_cierre) : null;
            $durStr = $dur
                ? sprintf('%dh %02dm', $dur->h + ($dur->days * 24), $dur->i) : 'En curso';
        @endphp
        ...
        <div class="cc-info-grid">
            ...
            <div class="cc-info-section">
                ...
                <dl class="cc-info-dl">
                    <dt>Caja</dt><dd>{{ $sesInfo->caja?->nombre ?? '—' }}</dd>
                    <dt>Cajero</dt><dd>{{ $sesInfo->cajero?->name ?? '—' }}</dd>
                    <dt>Apertura</dt><dd>{{ $sesInfo->fecha_apertura?->format('d/m/Y H:i:s') ?? '—' }}</dd>
                    <dt>Cierre</dt><dd>{{ $sesInfo->fecha_cierre?->format('d/m/Y H:i:s') ?? '—' }}</dd>
                    <dt>Duración</dt><dd>{{ $durStr }}</dd>
                    <dt>Estado</dt>
                    <dd>
                    ...
                    <tbody>
                        @foreach($sesInfo->pagos as $pag)
                            ...
                            <tr wire:key="pag-{{ $pag->id }}">
                                ...
                                <td class="rg-td-right"><span class="rg-monto">S/ {{ number_format($pag->importe_sistema, 2) }}</span></td>
                                <td class="rg-td-right">
