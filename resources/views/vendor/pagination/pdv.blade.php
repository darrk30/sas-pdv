...
<nav class="pdv-pag" role="navigation" aria-label="Paginación">
    ...
    <div class="pdv-pag__links">
        ...
        @else
            <a href="{{ $paginator->previousPageUrl() }}" class="pdv-pag__btn" wire:navigate>
                <svg viewBox="0 0 20 20" fill="currentColor" width="16" height="16"><path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
            </a>
        @endif
        ...
        @foreach ($elements as $element)
            ...
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    ...
                    @else
                        <a href="{{ $url }}" class="pdv-pag__btn" wire:navigate>{{ $page }}</a>
                    @endif
                @endforeach
            @endif
        @endforeach
        ...
        @if ($paginator->hasMorePages())
            <a href="{{ $paginator->nextPageUrl() }}" class="pdv-pag__btn" wire:navigate>
...
.dark .pdv-pag__info { color: rgb(148 163 184); }
...
.dark .pdv-pag__btn--disabled { color: rgb(71 85 105); }
</style>
